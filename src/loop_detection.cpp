...
void LoopDetector::addKeyFrame(KeyFrame* cur_kf, bool flag_detect_loop)
{
    ...
    if (loop_index != -1)
    {
        KeyFrame* old_kf = getKeyFrame(loop_index);//按照索引查找

        if (abs(cur_kf->time_stamp - old_kf->time_stamp) > MIN_LOOP_SEARCH_TIME)//30s
        {
            if (cur_kf->findConnection(old_kf))//描述子匹配，ORB和Hari的BRIEF描述子暴力匹配，pnp取外点，方差和校验
            {
...
int LoopDetector::detectLoop(KeyFrame* keyframe, int frame_index)
{
    ...
    db.query(keyframe->bow_descriptors, ret, 4, frame_index - MIN_LOOP_SEARCH_GAP);//50
