...
void KeyFrame::computeWindowOrbPoint()
{
    if (USE_ORB)
    {
        // ORB features
        vector<uchar> status;
        cv::Ptr<cv::ORB> detector = cv::ORB::create(NUM_ORB_FEATURES, 1.2f, 8, 1);//orb角点检测
        detector->detect(image_intensity, orb_window_keypoints, MASK);//检测orb角点
        keypointConverter(orb_window_keypoints, orb_point_2d_uv);//存储角点二维位置
        extractPoints(orb_point_2d_uv, orb_point_3d, orb_point_2d_norm, status);//in_out_out_out，由图像位置求解对应的3d坐标，归一化坐标，并筛选出不符合索引的部分坐标
        reduceVector(orb_point_3d, status);
        reduceVector(orb_point_2d_uv, status);
        reduceVector(orb_point_2d_norm, status);
        reduceVector(orb_window_keypoints, status);//筛去外点，感觉在extractPoints就可以做到
        detector->compute(image_intensity, orb_window_keypoints, orb_window_descriptors);//iio，计算brief描述子
    }
}
//计算shi  tomasi角点和brief描述子
void KeyFrame::computeWindowBriefPoint()
{
    if (USE_BRIEF)
    {
        // Corner feautres
        vector<uchar> status;
        cv::goodFeaturesToTrack(image_intensity, brief_point_2d_uv, NUM_BRI_FEATURES, 0.01, 10, MASK);//shi tomasi交点检测
        extractPoints(brief_point_2d_uv, brief_point_3d, brief_point_2d_norm, status);//去除外点
        reduceVector(brief_point_3d, status);
        reduceVector(brief_point_2d_uv, status);
        reduceVector(brief_point_2d_norm, status);
        keypointConverter(brief_point_2d_uv, brief_window_keypoints);//得到角点像素位置
        briefExtractor(image_intensity, brief_window_keypoints, brief_window_descriptors);//iio，brief描述子
    }
}
...
bool KeyFrame::findConnection(KeyFrame* old_kf)
{
    ...
    if (USE_ORB)
    {
        ...
        if ((int)good_matches.size() > MIN_LOOP_FEATURE_NUM)//15
        {
            vector<uchar> status;
    ...
    if (USE_BRIEF)
    {
        ...
        {
            status.clear();
            PnPRANSAC(matched_2d_old_norm, matched_3d, status);//去外点
            reduceVector(matched_3d, status);
            reduceVector(matched_2d_cur, status);
            reduceVector(matched_2d_old, status);
            reduceVector(matched_2d_old_norm, status);
...
//从像素点获得3d点云的位置，归一化坐标，关键点外点坐标，即去除部分不符合索引的关键点
...
bool KeyFrame::distributionValidation(const vector<cv::Point2f>& new_point_2d_uv, const vector<cv::Point2f>& old_point_2d_uv)
{
    ...
    pcl::compute3DCentroid(*new_cloud, new_xyz_centroid);//计算质心
    pcl::computeCovarianceMatrix(*new_cloud, new_xyz_centroid, new_covariance_matrix);//计算协方差 
